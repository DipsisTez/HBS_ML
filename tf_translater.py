...
def predicted_text(image_patch):
    image = DocumentFile.from_images(image_patch)
    result = model(image)
    return get_formated_text(result)
...
def translate_image_text(image_path, lang='en', to_lang='ru'):
    ...
    cv2.imwrite(BUFFER_IMAGE, cropped_img)

    text = predicted_text(BUFFER_IMAGE)
